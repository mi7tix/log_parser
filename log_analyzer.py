...
def last_log_search(log_dir):
    ...
    for logs in os.listdir(log_dir):
        ...
        try:
            ...
            logging.info("Log not found or {}".format(e))
    ...
    with gzip.open(path, 'rb') if path.endswith('.gz') else open(path) as base_log:
        ...
        for line in base_log:
            ...
            if parsed_line:
                handle_lines += 1
                yield parsed_line
        sucseccful_per = float(handle_lines) / float(total_lines) * 100
        logging.info("sucseccful percent {}".format(sucseccful_per))
